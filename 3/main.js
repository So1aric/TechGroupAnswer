...
const createRect = () => {
  ...
  gl.shaderSource(vertexShader, rectVertexShaderSrc);
  gl.compileShader(vertexShader);

  const fragmentShader = gl.createShader(gl.FRAGMENT_SHADER);
  gl.shaderSource(fragmentShader, rectFragmentShaderSrc);
  gl.compileShader(fragmentShader);

  console.log(gl.getShaderInfoLog(fragmentShader))

  const rectProgram = gl.createProgram();
  gl.attachShader(rectProgram, vertexShader);
  gl.attachShader(rectProgram, fragmentShader);
  gl.linkProgram(rectProgram);

  const iResolutionLocation = gl.getUniformLocation(rectProgram, "iResolution");
  const iTimeLocation = gl.getUniformLocation(rectProgram, "iTime");

  const beginTime = Date.now() / 1000 + Math.random() * 100;
  ...
  const render = () => {
    ...
    gl.uniform3f(iResolutionLocation, 200, 200, 0);
    gl.uniform1f(iTimeLocation, Date.now() / 1000 - beginTime);
...
const rectVertexBufferData = new Float32Array([
  -1, -1, 0, 0,
  1, -1, 200, 0,
  -1, 1, 0, 200,
  1, 1, 200, 200
